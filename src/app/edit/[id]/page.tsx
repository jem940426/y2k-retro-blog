...
import { updatePost } from '@/application/useCases/post';
...
export default function EditPage({ params }: EditPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (postData) {
        ...
        setEmoji(postData.emoji || '📝');
        // 에디터에 내용 주입은 contentRef가 마운트된 후 처리가 필요할 수도 있음
        // 하지만 use client 컴포넌트이며, 렌더링 시점에 inject는 어려우므로
        // 아래 렌더링 부분에서 dangerouslySetInnerHTML을 쓰거나
        // useEffect에서 innerHTML을 직접 넣어줌
  ...
  // contentRef가 늦게 로딩될 경우를 대비한 추가 effect
  useEffect(() => {
  ...
  const handleUpdate = async () => {
    ...
    // Supabase에 저장
    const { error } = await updatePost(id, title, htmlContent, category, emoji);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center w-full h-full text-[var(--color-y2k-pink-dark)] font-bold">
        Loading Data... ✿
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center justify-center w-full h-full relative z-10 px-8">
      {/* 장식 요소들 */}
      <div className="absolute top-[10%] left-[10%] text-[var(--color-y2k-pink-main)] text-3xl rotate-[-20deg] drop-shadow-sm pointer-events-none">✿</div>
      <div className="absolute top-[5%] right-[20%] text-[var(--color-y2k-pink-main)] text-2xl drop-shadow-sm pointer-events-none">✦</div>
      <div className="absolute bottom-[10%] left-[25%] text-[var(--color-y2k-pink-dark)] text-3xl rotate-[15deg] drop-shadow-sm pointer-events-none">💖</div>
      ...
      <div className="w-[750px] bg-[var(--color-y2k-pink-bg)] p-1 rounded-sm shadow-[8px_8px_0px_0px_rgba(255,102,163,0.4)] z-20">
        <div className="w-full bg-[#fdfafb] border-2 border-[var(--color-y2k-border)] flex flex-col h-[650px]">
          ...
          <div className="bg-[var(--color-y2k-pink-main)] px-2 py-1.5 flex justify-between items-center select-none">
            <span className="font-bold text-[10px] text-white tracking-widest leading-none drop-shadow-[1px_1px_0px_rgba(0,0,0,0.2)]">Edit.exe - 블로그 글 수정</span>
            <div className="flex gap-1">
              <button onClick={() => router.push(`/post/${id}`)} className="w-[14px] h-[14px] bg-[#ffe6f0] border border-[var(--color-y2k-border)] flex items-center justify-center font-black text-[8px] text-[var(--color-y2k-pink-dark)] leading-none hover:bg-white cursor-pointer">X</button>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col flex-1 pb-4">
            ...
            <div className="pt-3 border-t-2 border-dashed border-[var(--color-y2k-pink-main)] flex justify-between items-center">
              <span className="text-[9px] font-bold text-[#888] tracking-widest bg-[#ffe6f0] px-2 py-1">Mode: Edit Mode</span>
